            ...
            properties: [
                [{
                    ...
                    ,
                    {
                        ...
                        ,name: 'maskInactive'
                        ,type: 'colorpicker'
                        ,value: function(item,name) { if(item.maskInactive === undefined) return '#ffffff'; return item.maskInactive; }
                        ,hiddenByDefault: function(item,name){
                    ...
                    ,
                    {
                        ...
                        ,name: 'maskActive'
                        ,type: 'colorpicker'
                        ,value: function(item,name) { if(item.maskActive === undefined) return '#ffffff'; return item.maskActive; }
                        ,hiddenByDefault: function(item,name){
                            return (!/icon/i.test(item.name) || (item.maskEnabled !== undefined && item.maskEnabled === false));
                        }
                        ,liveUpdate: 'background-color'
...
prx.types.material_textfield = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(prx.editor) {
            ...
            cR += '<div class="material-textfield-field-wrapper liveUpdate-backgroundColor-background-color">';
            cR += '<div class="faux-input liveUpdate-textColor-color liveUpdate-borderColor-border-color changeProperty-textColor changeProperty-borderColor changeProperty-textFontStyle" data-editableproperty="value">'+prx.componentsHelper.getProp(item.value,'text')+'</div>';
            cR += '<div class="faux-input placeholder-input liveUpdate-borderColor-border-color  liveUpdate-placeholderColor-color changeProperty-textColor changeProperty-borderColor">'+prx.componentsHelper.getProp(item.placeholder,'other')+'</div>';
            cR += '</div>';
        }
        else {
            if(prx.componentsHelper.getProp(item.hasIcon,'boolean') && item.imgSrc.fileid != '') {
            ...
            cR += '<input autocomplete="new-password" data-lpignore="true" type="'+prx.componentsHelper.getProp(item.inputtype,'other')+'" value="'+prx.componentsHelper.getProp(item.value,'text')+'" placeholder="'+prx.componentsHelper.getProp(item.placeholder,'other')+'" data-role="none" class="real-input liveUpdate-backgroundColor-background-color liveUpdate-textColor-color liveUpdate-borderColor-border-color changeProperty-backgroundColor changeProperty-textColor changeProperty-textSize changeProperty-textFont changeProperty-textFontFontStyle changeProperty-borderColor changeProperty-borderWidth changeProperty-borderRadius changeProperty-textAlign" />'
            cR += '</div>';
        }
        cR += prx.items.getComponentAppendDivs(item, containerid, symbol);
        cR += '</div>';
...
prx.types.material_textarea = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        var cR = '';
        cR += '<div id="' + _id + '" ' + prx.items.getComponentBaseAttributes(item, containerid, symbol)  + ' class="' + prx.items.getComponentBaseClasses(item, containerid, symbol) + ' box pos type-material-textarea'+((prx.componentsHelper.getProp(item.isFull,'boolean')) ? ' full' : '')+' ">';
        cR += '<div class="material-textarea-bg-wrapper liveUpdate-backgroundColor-background-color changeProperty-backgroundColor">';

        cR += '<style>';
        cR += prx.items.getComponentBaseStyle(item, containerid, symbol);
        //cR += '#'+_id+' { '+prx.utils.getBgCss(item.backgroundColor)+';}';
		// cR += '#'+_id+' { background-color: '+prx.componentsHelper.getProp(item.backgroundColor,'color-bakground')+';}';
		cR += '#'+_id+' .material-textarea-bg-wrapper > textarea, #'+_id+' .faux-input { padding-top: '+(16*prx.componentsHelper.getScale(item.lib))+'px; color: '+prx.componentsHelper.getProp(item.textColor,'color-text')+'; font-size: '+prx.componentsHelper.getProp(item.textSize,'num-text-size')+'px; '+prx.componentsHelper.getProp(item.textFont,'font-family')+' border-bottom: '+prx.componentsHelper.getProp(item.borderWidth,'num-border-width')+'px solid '+prx.componentsHelper.getProp(item.borderColor,'color-border')+'; text-align: '+prx.componentsHelper.getProp(item.textAlign,'align')+'; border-radius: '+prx.componentsHelper.getProp(item.borderRadius,'border-radius')+' '+ _props + '}';
        ...
        cR += '#'+_id+' .material-textarea-bg-wrapper { width: 100%; height: 100%; '+prx.gradients.getBgCssByProperty(item.backgroundColor,'color-background')+'; }';if(prx.componentsHelper.getProp(item.isFull,'boolean')) {
        ...
        if(prx.editor) {
            ...
            cR += '<div class="faux-input liveUpdate-textColor-color liveUpdate-borderColor" data-editableproperty="value">'+prx.componentsHelper.getProp(item.value,'text-textarea')+'</div>';
            cR += '<div class="faux-input placeholder-input liveUpdate-placeholderColor-color liveUpdate-borderColor-border-color">'+prx.componentsHelper.getProp(item.placeholder,'other')+'</div>'
        } else {
            if(prx.componentsHelper.getProp(item.hasLabel,'boolean')) {
                cR += '<div class="label liveUpdate-labelColor-color changeProperty-labelColor changeProperty-labelFont changeProperty-labelFontStyle changeProperty-labelSize changeProperty-labelAlign" data-editableproperty="label">'+prx.componentsHelper.getProp(item.label,'text-textarea')+'</div>';
            }
...
prx.types.material_textfield_floating_label = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(!prx.editor) {
            cR += '#'+_id+' input:focus, #'+_id+' textarea:focus { border-bottom: '+prx.componentsHelper.getProp(item.borderWidth,'num-border-width')*2+'px solid; border-color: '+prx.componentsHelper.getProp(item.borderColorActive,'color-border')+'; }';
        ...
        if(prx.editor) {
            ...
            cR += '<div class="faux-input '+((prx.componentsHelper.getProp(item.fieldtype,'other')== 'textarea') ? 'textarea ' : '')+'liveUpdate-textColor-color liveUpdate-borderColor-border-color liveUpdate-backgroundColor-background-color changeProperty-textProperties changeProperty-textFontStyle" data-editableproperty="value">'+((prx.componentsHelper.getProp(item.fieldtype,'other') != 'textarea')  ? prx.componentsHelper.getProp(item.value,'text') : prx.componentsHelper.getProp(item.value,'text-textarea')) +'</div>';
